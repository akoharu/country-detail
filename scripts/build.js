...
const getFlagEmojis = async (pretty = false) => {
  const data = await fs.readFile('data/emoji-sequences.txt', 'utf8');
    // get phone codes from dist/country-codes.json
  const phoneCodes = await fs.readJson('dist/country-codes.json');
  ...
  const emojis = data.split('\n').map(row => {
    ...
    const code = codePoints.length === 2
      ...
      : name.toUpperCase();
    // get phone code by contry code
    const phoneCode = phoneCodes[code] ? phoneCodes[code] : null;
    console.log(`${code} ${name} ${phoneCode}`);
    return {
      name,
      code,
      phoneCode,
...
// get contry phone codes with axios
const getCountryCodes = async () => {
    const res = await axios.get('http://country.io/phone.json');
    const countryCodes = res.data;
    // save to file
    await fs.outputJson('dist/country-codes.json', countryCodes, { spaces: 2 });
}
